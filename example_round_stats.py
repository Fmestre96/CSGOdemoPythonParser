...
def analyse_demo(path):
    file = open(path, "rb")
    x = dp.DemoParser(file, ent="NONE")

    
    x.subscribe_to_event("parser_start", new_demo)
    x.subscribe_to_event("gevent_player_team", player_team)
    x.subscribe_to_event("gevent_player_death", player_death)
    x.subscribe_to_event("gevent_player_spawn", player_spawn)
    x.subscribe_to_event("gevent_bot_takeover", bot_takeover)
    x.subscribe_to_event("gevent_begin_new_match", begin_new_match)
    x.subscribe_to_event("gevent_round_end", round_end)
    x.subscribe_to_event("gevent_round_officially_ended", round_officially_ended)
    x.subscribe_to_event("parser_update_pinfo", update_pinfo)
    x.subscribe_to_event("cmd_dem_stop", cmd_dem_stop)
    x.subscribe_to_event("parser_demo_finished_print", print_match_stats)
